...
Local release checks: production build and all 48 unit tests passed; 29 relevant browser scenarios passed across the focused runs after correcting a test-fixture alias. Visual inspection confirmed all three city levels receive their compounds, and a fuel blast destroys a facing wall while that wall shields an enemy behind it for the same blast. The full 86-scenario browser suite runs in the deployment workflow before publishing.

A scripted 660-enemy Crazy-city battle on this PC averaged 2.30 ms (Low) and 2.19 ms (High) per simulation update, with 26/42 draw calls at the sampled frame. The test advances 120 simulation frames and checks bounded projectiles, fragments and corpses. These are local CPU/render-work observations, not end-to-end FPS or real-phone measurements.
