...
test("phone quick start hides advanced text but keeps controls accessible", async ({
  ...
  await expect(page.locator(".manual-section")).toHaveCount(4);
  ...
  const counters = page.getByText(/^Riflemen keep their distance/);
  await expect(counters).toBeHidden();
  await page.getByText("Enemy soldiers & counters", { exact: true }).tap();
  await expect(counters).toBeVisible();
  await expect(page.locator(".manual-section[open]")).toHaveCount(2);
  await page.getByText("Enemy soldiers & counters", { exact: true }).tap();
  await expect(counters).toBeHidden();
